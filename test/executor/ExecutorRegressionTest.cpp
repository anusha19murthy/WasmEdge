...
/// Regression test for missing NaN guard in the f64x2->i32x4 trunc_sat path.
///
/// (module
///   (func (export "test_s") (result i32)
///     v128.const i64x2 0x7FF8000000000000 0x7FF8000000000000
///     i32x4.trunc_sat_f64x2_s_zero
///     i32x4.extract_lane 0)
///   (func (export "test_u") (result i32)
///     v128.const i64x2 0x7FF8000000000000 0x7FF8000000000000
///     i32x4.trunc_sat_f64x2_u_zero
///     i32x4.extract_lane 0))
std::array<WasmEdge::Byte, 98> TruncSatF64x2NaNWasm{
    0x00, 0x61, 0x73, 0x6D, 0x01, 0x00, 0x00, 0x00, 0x01, 0x05, 0x01,
    0x60, 0x00, 0x01, 0x7F, 0x03, 0x03, 0x02, 0x00, 0x00, 0x07, 0x13,
    0x02, 0x06, 0x74, 0x65, 0x73, 0x74, 0x5F, 0x73, 0x00, 0x00, 0x06,
    0x74, 0x65, 0x73, 0x74, 0x5F, 0x75, 0x00, 0x01, 0x0A, 0x37, 0x02,
    0x1A, 0x00, 0xFD, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF8,
    0x7F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF8, 0x7F, 0xFD, 0xFC,
    0x01, 0xFD, 0x1B, 0x00, 0x0B, 0x1A, 0x00, 0xFD, 0x0C, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0xF8, 0x7F, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0xF8, 0x7F, 0xFD, 0xFD, 0x01, 0xFD, 0x1B, 0x00, 0x0B};

TEST(ExecutorRegression, TruncSatF64x2NaNProducesZero) {
  Configure Conf;
  VM::VM VM(Conf);
  ASSERT_TRUE(VM.loadWasm(TruncSatF64x2NaNWasm));
  ASSERT_TRUE(VM.validate());
  ASSERT_TRUE(VM.instantiate());

  auto Rs = VM.execute("test_s");
  ASSERT_TRUE(Rs);
  ASSERT_EQ((*Rs).size(), 1U);
  EXPECT_EQ((*Rs)[0].first.get<int32_t>(), 0)
      << "i32x4.trunc_sat_f64x2_s_zero on NaN must produce 0";

  auto Ru = VM.execute("test_u");
  ASSERT_TRUE(Ru);
  ASSERT_EQ((*Ru).size(), 1U);
  EXPECT_EQ((*Ru)[0].first.get<uint32_t>(), 0U)
      << "i32x4.trunc_sat_f64x2_u_zero on NaN must produce 0";
}
